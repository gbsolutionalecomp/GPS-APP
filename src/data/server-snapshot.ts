...
async function mapEvidenceRows(supabase: SupabaseClient, rows: Row[]): Promise<OdometerEvidence[]> {
  ...
  const signed = paths.length
    ...
    : { data: null }
  const items = (signed.data ?? []) as SignedUrlResult[]
  const urlByPath = new Map<string, string>(
    items.filter((item) => item.signedUrl && !item.error).map((item) => [item.path, item.signedUrl!])
  )
  return rows.map((row) => {
